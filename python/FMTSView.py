...
class FMTS_View(QWidget):
    def __init__(self,parent):
        ...
        # layout = QVBoxLayout()
        #
        # btn_prev = QPushButton("Back to Page 1")
        # btn_prev.clicked.connect(self.go_to_page1)
    ...
    def initUI(self):
        ...
        button = QPushButton("첫 번째 페이지로 이동")
        button.clicked.connect(lambda: self.parent.switch_page(1))  # 1번째 페이지로 이동

        layout.addWidget(button)
        self.setLayout(layout)

    def go_to_page1(self):
        self.stacked_widget.setCurrentIndex(0)  # 첫 번째 페이지로 전환
